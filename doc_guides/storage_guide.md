# 💾 Fluera Engine — Storage Guide

How to persist canvases locally using the Fluera Engine SDK.
  ...
  onCanvasSelected: (canvasId) {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => FlueraCanvasScreen(
        config: FlueraCanvasConfig(
          storageAdapter: storage,
          layerController: FlueraLayerController(),
        ),
        canvasId: canvasId,
      ),
    ));
...
Implement `FlueraStorageAdapter` to use **any** backend:

```dart
import 'package:fluera_engine/fluera_engine.dart';

class MyCustomStorage implements FlueraStorageAdapter {
  @override
...
class HiveStorageAdapter implements FlueraStorageAdapter {
  late Box<String> _box;

  @override
  Future<void> initialize() async {
    await Hive.initFlutter();
    _box = await Hive.openBox<String>('fluera_canvases');
  }
...
class DriftStorageAdapter implements FlueraStorageAdapter {
...
class FirestoreStorageAdapter implements FlueraStorageAdapter {
  final String userId;
