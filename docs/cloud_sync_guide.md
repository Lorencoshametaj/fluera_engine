...
Nebula Engine supporta il salvataggio automatico dei canvas su qualsiasi database online tramite l'interfaccia `NebulaCloudStorageAdapter`.

## Come funziona

```
Canvas modificato → auto-save locale → NebulaSyncEngine (debounce 3s) → NebulaCloudStorageAdapter → tuo backend
```

1. **L'utente disegna** — il canvas si salva localmente
2. **Dopo 3 secondi di inattività** — il `NebulaSyncEngine` invia i dati al cloud
...
Crea una classe che implementa `NebulaCloudStorageAdapter`:

```dart
import 'package:nebula_engine/nebula_engine.dart';

class MyCloudAdapter implements NebulaCloudStorageAdapter {
  @override
...
```dart
class SupabaseCloudAdapter implements NebulaCloudStorageAdapter {
...
```dart
class FirestoreCloudAdapter implements NebulaCloudStorageAdapter {
...
```dart
class RestCloudAdapter implements NebulaCloudStorageAdapter {
...
Il `NebulaSyncEngine` gestisce internamente:
