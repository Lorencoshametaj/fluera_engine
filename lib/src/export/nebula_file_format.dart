...
// =============================================================================
// 📦 NEBULA FILE FORMAT v4 — Sectioned binary format with lazy loading
//
// Structure:
...
/// Types of sections in a Nebula file.
enum SectionType {
...
/// The Nebula file header (32 bytes).
///
/// Layout:
...
class NebulaFileHeader {
  final int version;
  final int flags;
  final int sectionCount;
  final int tocOffset;

  const NebulaFileHeader({
  ...
  factory NebulaFileHeader.decode(Uint8List bytes) {
    if (bytes.length < 32) {
      throw const FormatException('File too small for Nebula header');
    }
  ...
  /// Check if raw bytes start with the Nebula magic.
  static bool isNebulaFile(Uint8List bytes) {
...
class NebulaFileTOC {
  ...
  const NebulaFileTOC(this.sections);
...
// ─────────────────────────────────────────────────────────────────────────────
// NEBULA FILE WRITER — Build/write Nebula files
// ─────────────────────────────────────────────────────────────────────────────
...
/// Writes Nebula files with support for incremental saves.
///
/// ```dart
...
class NebulaFileWriter {
  ...
  Uint8List build() {
    ...
    final toc = NebulaFileTOC(descriptors);
  ...
  static Uint8List incrementalSave({
    ...
    // Parse existing file
    final header = NebulaFileHeader.decode(existingBytes);
    final tocBytes = Uint8List.sublistView(
      existingBytes,
      header.tocOffset,
      header.tocOffset + header.sectionCount * 28,
    );
    final toc = NebulaFileTOC.decode(tocBytes, header.sectionCount);
...
// ─────────────────────────────────────────────────────────────────────────────
// NEBULA FILE READER — Read/parse Nebula files with lazy section loading
// ─────────────────────────────────────────────────────────────────────────────

/// Reads Nebula files with lazy, section-level access.
///
/// ```dart
/// final reader = NebulaFileReader(fileBytes);
...
/// ```
class NebulaFileReader {
  final Uint8List _bytes;
  final NebulaFileHeader header;
  final NebulaFileTOC toc;

  NebulaFileReader._(this._bytes, this.header, this.toc);

  factory NebulaFileReader(Uint8List bytes) {
    final header = NebulaFileHeader.decode(bytes);
    ...
    final toc = NebulaFileTOC.decode(tocBytes, header.sectionCount);
    return NebulaFileReader._(bytes, header, toc);
  }
  ...
  /// Whether this is a valid Nebula file.
  static bool isValid(Uint8List bytes) => NebulaFileHeader.isNebulaFile(bytes);
