...
// ============================================================================
// NEON GLOW SHADER RENDERER — GPU multi-layer bloom
// ============================================================================

extension ShaderNeonGlowRenderer on ShaderBrushService {
  /// Render neon glow stroke with GPU multi-layer bloom and bright core.
  void renderNeonGlowPro(
    Canvas canvas,
    List<dynamic> points,
    Color color,
    double baseWidth, {
    double opacity = 1.0,
    double glowRadius = 0.6,
  }) {
    if (!isAvailable || neonGlowShader == null || points.length < 2) return;

    final shader = neonGlowShader!;
    final paint =
        Paint()
          ..shader = shader
    ...
    for (int k = 0; k < offsets.length - 1; k++) {
      ...
      final w1 = glowWidth * (0.5 + press * 0.5);
      final w2 = glowWidth * (0.5 + getPressure(points[k + 1]) * 0.5);

      // Wide extend for bloom effect
      final bloomExtend = (w1 + w2) * 0.5 * (1.5 + glowRadius * 2.0);
      if (rawSegLen > 0.01) {
        final dir = (p2 - p1) / rawSegLen;
        p1 = p1 - dir * bloomExtend * 0.3;
        p2 = p2 + dir * bloomExtend * 0.3;
      }

      final maxW = math.max(w1, w2) * (2.0 + glowRadius * 3.0);
      final padding = maxW * 0.5 + 8.0;
      ...
      shader.setFloat(idx++, glowRadius);
