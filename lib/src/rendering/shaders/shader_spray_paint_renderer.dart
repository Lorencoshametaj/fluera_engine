...
// ============================================================================
// SPRAY PAINT SHADER RENDERER — GPU stochastic dot spatter
// ============================================================================

extension ShaderSprayPaintRenderer on ShaderBrushService {
  /// Render spray paint stroke with GPU stochastic dots and gaussian falloff.
  void renderSprayPaintPro(
    Canvas canvas,
    List<dynamic> points,
    Color color,
    double baseWidth, {
    ...
    for (int k = 0; k < offsets.length - 1; k++) {
      ...
      final w1 = sprayWidth * (0.4 + press * 0.6);
      final w2 = sprayWidth * (0.4 + getPressure(points[k + 1]) * 0.6);

      // Wider extend for spray spread
      final halfW = (w1 + w2) * 0.3;
      if (rawSegLen > 0.01) {
      ...
      final maxW = math.max(w1, w2) * 1.3;
      final padding = maxW * 0.5 + 6.0;
