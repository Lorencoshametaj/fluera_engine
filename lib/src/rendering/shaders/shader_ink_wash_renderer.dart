...
// ============================================================================
// INK WASH SHADER RENDERER — GPU sumi-e wet ink diffusion
// ============================================================================

extension ShaderInkWashRenderer on ShaderBrushService {
  /// Render ink wash stroke with GPU wet-ink diffusion and bleed edges.
  void renderInkWashPro(
    Canvas canvas,
    List<dynamic> points,
    Color color,
    double baseWidth, {
    ...
    for (int k = 0; k < offsets.length - 1; k++) {
      ...
      final w1 = washWidth * (0.5 + press * 0.5);
      final w2 = washWidth * (0.5 + getPressure(points[k + 1]) * 0.5);

      // Extend with wetness influence
      final bleedExtend = (w1 + w2) * 0.25 * (1.0 + wetness * 0.3);
      if (rawSegLen > 0.01) {
        final dir = (p2 - p1) / rawSegLen;
        p1 = p1 - dir * bleedExtend;
        p2 = p2 + dir * bleedExtend;
      }

      final maxW = math.max(w1, w2) * (1.0 + wetness * 0.4);
      final padding = maxW * 0.5 + 5.0;
