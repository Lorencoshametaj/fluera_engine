...
// =============================================================================
// ☁️ NEBULA CLOUD STORAGE ADAPTER — Abstract cloud persistence interface
//
...
enum NebulaSyncState { idle, syncing, error }
...
/// }
/// ```
abstract class NebulaCloudStorageAdapter {
...
// =============================================================================
// ⚙️ NEBULA SYNC ENGINE — Internal orchestrator for cloud auto-save
//
...
class NebulaSyncEngine {
  final NebulaCloudStorageAdapter _adapter;

  /// Expose the adapter for direct asset upload/download operations.
  NebulaCloudStorageAdapter get adapter => _adapter;
  ...
  NebulaSyncEngine({
    required NebulaCloudStorageAdapter adapter,
  ...
  Future<void> _executeSave() async {
    ...
    state.value = NebulaSyncState.syncing;
    lastError.value = null;

    try {
      await _adapter.saveCanvas(canvasId, data);
      _retryCount = 0;
      state.value = NebulaSyncState.idle;
      ...
      if (_retryCount <= maxRetries) {
        ...
        state.value = NebulaSyncState.error;
