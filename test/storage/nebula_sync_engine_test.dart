// NebulaSyncEngine uses Flutter's ValueNotifier, so flutter_test is needed.
...
import 'package:nebula_engine/src/storage/nebula_cloud_adapter.dart';

// =============================================================================
// 🧪 NebulaSyncEngine Unit Tests
// =============================================================================

// ─── Mock Cloud Adapter ─────────────────────────────────────────────────

/// A controllable mock adapter for testing save/load/delete behavior.
class MockCloudAdapter implements NebulaCloudStorageAdapter {
  int saveCallCount = 0;
  int loadCallCount = 0;
...
void main() {
  ...
  late NebulaSyncEngine engine;

  setUp(() {
    adapter = MockCloudAdapter();
    engine = NebulaSyncEngine(
      adapter: adapter,
  ...
  group('NebulaSyncEngine — State', () {
    test('initial state is idle', () {
      expect(engine.state.value, NebulaSyncState.idle);
      expect(engine.lastError.value, isNull);
    });
  });

  group('NebulaSyncEngine — flush (immediate save)', () {
    ...
    test('flush transitions state through syncing', () async {
      final states = <NebulaSyncState>[];
      engine.state.addListener(() => states.add(engine.state.value));

      await engine.flush('c1', {'k': 'v'});

      expect(states, contains(NebulaSyncState.syncing));
      expect(engine.state.value, NebulaSyncState.idle);
    });
  });

  group('NebulaSyncEngine — requestSave (debounced)', () {
  ...
  group('NebulaSyncEngine — Load & Delete passthrough', () {
  ...
  group('NebulaSyncEngine — Error handling', () {
    test('enters error state after maxRetries exhausted', () async {
      ...
      expect(
        engine.state.value,
        anyOf(NebulaSyncState.syncing, NebulaSyncState.error),
      );
    });
  ...
  group('NebulaSyncEngine — Overlapping saves', () {
  ...
  group('NebulaSyncEngine — Asset upload/download', () {
